...
PBDSimulation::PBDSimulation(const Settings& settings) : m_settings(settings) {
  ...
  if (loadSDFObstacle(obsfilepath)) {
    ...
    for (unsigned int i = 0; i < tms.size(); i++) {
      ...
      m_cd2.addCollisionObjectWithoutGeometry(
          i,
          PBD::CollisionDetection::CollisionObject::
              TriangleModelCollisionObjectType,
          &pd.getPosition(offset), nVert, true);

      tm->setRestitutionCoeff(0.05f);  // 0.6
      tm->setFrictionCoeff(0.1f);      // 0.2
      m_model->setContactStiffnessParticleRigidBody(
          m_settings.contact_stiffness);
    }
  }

  // set cloth stiffnesses
  m_model->setValue(PBD::SimulationModel::CLOTH_STIFFNESS_XX,
                    m_settings.stiffness[0]);
  m_model->setValue(PBD::SimulationModel::CLOTH_STIFFNESS_XY,
                    m_settings.stiffness[1]);
  m_model->setValue(PBD::SimulationModel::CLOTH_STIFFNESS_YY,
                    m_settings.stiffness[2]);
  m_model->setValue(PBD::SimulationModel::CLOTH_BENDING_STIFFNESS,
                    m_settings.bending_stiffness);
  // for distance constraint stiffness just use average
  m_model->setValue(PBD::SimulationModel::CLOTH_STIFFNESS,
                    (m_settings.stiffness[0] + m_settings.stiffness[1] +
                     m_settings.stiffness[2]) /
                        3);
  // skipping:
  ...
  PBD::Simulation::getCurrent()->setVecValue<Real>(PBD::Simulation::GRAVITATION,
                                                   m_settings.gravity);
...
bool PBDSimulation::loadClothMesh(const std::string& filepath) {
  ...
  m_model->addTriangleModel(X.size(), F.size(), &points[0], &indices[0],
                            uvIndices, uvs);

  // calculate vertex mass
  // TODO currently vertex mass doesnt do anything! maybe just remove
  // computation then...
  ...
  // hard-coded vertex selection for applyForce
  // TODO select by mouse or load from file, ...
  ...
  // // TODO set fixed verts for other simulations (imgui, by height or list of
  // vertex ids, or both, default none)
  ...
  for (unsigned int cm = 0; cm < m_model->getTriangleModels().size(); cm++) {
    ...
    if (m_settings.simulationMethod == 1) {
      ...
      const Utilities::IndexedFaceMesh::Edge* edges =
          trimodel->getParticleMesh().getEdges().data();
      ...
      for (unsigned int i = 0; i < nFacesUV; i++) {
        ...
        PBD::UVStrainTriangleConstraint* c =
            new PBD::UVStrainTriangleConstraint();
    ...
    if (m_settings.bendingMethod != 0) {
      ...
      const PBD::TriangleModel::ParticleMesh::Edge* edges =
          mesh.getEdges().data();
      const unsigned int* tris = mesh.getFaces().data();
      for (unsigned int i = 0; i < nEdges; i++) {
        ...
        if ((tri1 != 0xffffffff) && (tri2 != 0xffffffff)) {
          ...
          int point2           = -1;
          for (int j = 0; j < 3; j++) {
            if ((tris[3 * tri1 + j] != axisPoint1) &&
                (tris[3 * tri1 + j] != axisPoint2)) {
              point1 = tris[3 * tri1 + j];
              break;
            }
          }
          for (int j = 0; j < 3; j++) {
            if ((tris[3 * tri2 + j] != axisPoint1) &&
                (tris[3 * tri2 + j] != axisPoint2)) {
          ...
          if ((point1 != -1) && (point2 != -1)) {
            ...
            if (m_settings.bendingMethod == 1)
              m_model->addDihedralConstraint(vertex1, vertex2, vertex3,
                                             vertex4);
            else if (m_settings.bendingMethod == 2)
              m_model->addIsometricBendingConstraint(vertex1, vertex2, vertex3,
                                                     vertex4);
          }
        }
      }
...
bool PBDSimulation::loadBoxObstacle(const std::string& filepath) {
  ...
  {  // obstacle mesh to pbd rigidbody mesh
    ...
    vd.reserve(nPoints);
    for (unsigned int i = 0; i < nPoints; i++) vd.addVertex(oX[i].map());
    for (unsigned int i = 0; i < nFaces; i++) {
      int posIndices[3];
      auto f = oF[i].map();
      for (int j = 0; j < 3; j++) posIndices[j] = f[j];
    ...
    rb[0]->initBody(
        ...
        Vector3r(0.0, -0.095792, -0.0477), Quaternionr(1.0, 0.0, 0.0, 0.0), vd,
        mesh,
    ...
    m_obstacles[0].transformation.translation =
        Vector3r(0.0, -0.095792, -0.0477);

    const std::vector<Vector3r>* vertices1 =
        rb[0]->getGeometry().getVertexDataLocal().getVertices();
    const unsigned int nVert1 = static_cast<unsigned int>(vertices1->size());
    m_cd.addCollisionBox(
        0,  // rigid body index
        PBD::CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
        &(*vertices1)[0], nVert1, Vector3r(0.07, 0.07, 0.07));
  }
...
bool PBDSimulation::loadSDFObstacle(const std::string& filepath) {
  ...
  // NOTE: there might be some redundancy/useless copies of mesh data between
  // rigidbody and sdf etc. that could be avoided
  ...
  auto rb = rbs.back();
  rb->initBody(1.0, Vector3r(0.0, 0.0, 0.0), Quaternionr(1.0, 0.0, 0.0, 0.0),
               vd, mesh, Vector3r(1.0, 1.0, 1.0));  // scale
  rb->setMass(0.0);                                 // static

  std::vector<unsigned int>& faces = mesh.getFaces();

  // Discregrid::TriangleMesh sdfMesh(&vd.getPosition(0)[0], faces.data(),
  // vd.size(), nFaces); if type is float, copy vector to double vector
  std::vector<double> doubleVec;
  doubleVec.resize(3 * vd.size());
  for (unsigned int i = 0; i < vd.size(); i++)
    for (unsigned int j = 0; j < 3; j++)
      doubleVec[3 * i + j] = vd.getPosition(i)[j];
  Discregrid::TriangleMesh sdfMesh(&doubleVec[0], faces.data(), vd.size(),
                                   nFaces);
  ...
  auto resolution         = std::array<unsigned int, 3>(
      {static_cast<unsigned int>(float(extents[0]) *
                                 m_settings.sdf_samples_per_m) +
           1,
       static_cast<unsigned int>(float(extents[1]) *
                                 m_settings.sdf_samples_per_m) +
           1,
       static_cast<unsigned int>(float(extents[2]) *
                                 m_settings.sdf_samples_per_m) +
           1});
  ...
  m_rbsdfs.push_back(std::make_shared<PBD::CubicSDFCollisionDetection::Grid>(
      domain, resolution));
  ...
  m_cd2.addCubicSDFCollisionObject(
      m_rbsdfs.size() - 1,
      PBD::CollisionDetection::CollisionObject::RigidBodyCollisionObjectType,
      &(*vertices)[0], nVert, m_rbsdfs.back(), scale, testMesh, invertSDF);
...
void PBDSimulation::update() {
  ...
  PBD::Simulation::getCurrent()->getTimeStep()->setValue(
      PBD::TimeStepController::MAX_ITERATIONS,
      static_cast<unsigned int>(m_settings.iterations));
...
void PBDSimulation::applyForce(float fx, float fy, float fz) {
  ...
  for (auto i : m_selected) {
    pd.getVelocity(i) +=
        factor * pd.getInvMass(i) * h * force;  // note setting, not adding
