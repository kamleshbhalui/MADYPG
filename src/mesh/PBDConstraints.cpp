...
bool UVFEMTriangleConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter)
{
  ParticleData &pd = model.getParticles();
...
bool UVStrainTriangleConstraint::initConstraint(SimulationModel &model, const Vector2r& u0, const Vector2r& u1, const Vector2r& u2)
{
  ...
  // return false;
	...
	Real a = u1[0] - u0[0]; Real b = u2[0] - u0[0];
	Real c = u1[1] - u0[1]; Real d = u2[1] - u0[1];
	...
	m_invRestMat(0,0) =  d*s;  m_invRestMat(0,1) = -b*s;
	m_invRestMat(1,0) = -c*s;  m_invRestMat(1,1) =  a*s;

	return true;
}

bool UVStrainTriangleConstraint::solvePositionConstraint(SimulationModel &model, const unsigned int iter)
{
  ParticleData &pd = model.getParticles();
...
// 			r[2] = Vector3r(p1[2] - p0[2], p2[2] - p0[2], 0.0);
			...
			r[0] = Vector2r((p1[0] + corr1[0]) - (p0[0] + corr0[0]), (p2[0] + corr2[0]) - (p0[0] + corr0[0]));		// Gauss - Seidel
			r[1] = Vector2r((p1[1] + corr1[1]) - (p0[1] + corr0[1]), (p2[1] + corr2[1]) - (p0[1] + corr0[1]));
			r[2] = Vector2r((p1[2] + corr1[2]) - (p0[2] + corr0[2]), (p2[2] + corr2[2]) - (p0[2] + corr0[2]));
